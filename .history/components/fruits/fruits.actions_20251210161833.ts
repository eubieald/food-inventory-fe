...
import { AddFruitsDataType, FruitsDataType } from "./fruits.types";
...
export const getFruitsList = async ({ searchParams }: FruitsListProps) => {
  ...
  console.log("api response data", data); // My api is working afterall i just dont have enough time to destructure the data with their correct types
  return data;
};

// export const addFruits = async (data: AddFruitsDataType) => {
const path = `fruits`;
const response = await genericRequest({
  path: path,
  method: "POST",
